...
export default class Bott extends Botw {
    ...
    makeeGroup(msg) {
        ...
        for (let x of message.message) if (x.type === "at" && x.qq === Bot.uin) {
            atme = true
            break
        }
        ...
        return {
            ...
            group: {
                recallMsg: (msg_id) => {
                    this.delChannelMsg(msg.channel_id, msg_id, true)
                }
            },
            recall: () => {
                this.delChannelMsg(msg.channel_id, msg.id, true)
            }
        }
    }

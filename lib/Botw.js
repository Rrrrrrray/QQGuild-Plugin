...
export default class Botw {
    ...
    outLogN(...logs) {
        Bot.logger.info(...this.#makeLog(...logs))
    }

    outLogW(...logs) {
        Bot.logger.warn(...this.#makeLog(...logs))
    }

    outLogE(...logs) {
        Bot.logger.error(...this.#makeLog(...logs))
    }

    outLogD(...logs) {
        Bot.logger.debug(...this.#makeLog(...logs))
    }
    ...
    async callApi(url, option = {}) {
        ...
        return await fetch(this.#apiUrl + url, option).then((r) => r.json()).catch((error) => {
            this.outLogE(error)
        })
    }
}

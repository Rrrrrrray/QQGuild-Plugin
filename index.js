...
export class QQGuild extends Plugin {
    ...
    async 控制台(e) {
        ...
        if (global.__QQGuild__ !== true) {
            e.reply("出现未知错误！")
            Bot.logger.warn("警告！警告！警告！出现未知错误！")
            return
        }
...
if (global.__QQGuild__ === undefined) {
    ...
    Bot.logger.info(`------------------------`)
    Bot.logger.info(`- 欢迎使用QQ频道插件(${name}) v${version}`)
    Bot.logger.info(`- 本插件作者：${author}`)
    Bot.logger.info(`- 正在初始化... ^_^`)
    ...
    Bot.logger.info("- 发现机器人配置", `[${fes.length}个]`)
    Bot.logger.info(`------------------------`)
